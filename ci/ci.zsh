...
process_computation()
{
  ...
  PROMPT_VERSION=$1
  zsh -c "${PROMPT_DIR}/prompt_ci.zsh"
  for ((i=0;i<${nb_run};i++))
  do
    ns=$(${PROMPT_DIR}/prompt_ci.zsh | grep "ms" | grep "ns" | cut -d "=" -f 1 | sed -e "s/ns//g" )
    IFS=" " read -r -A ns <<< "${ns}"
    ns=${ns[${#ns[@]}]}
    ns=${ns#0}
    if [[ ${ns} -lt 0 ]]
...
main()
{
  ...
  sed "s/env bash/env zsh/g" ${PROMPT_DIR}/prompt.sh  > ${PROMPT_DIR}/prompt_ci.zsh
  echo "\
SHELL=\"/bin/zsh\"
CI=true
DEBUG_MODE=\"true\"
DEBUG_LEVEL=\"TIME\"
precmd
echo -e \"\${PROMPT}\""  >> ${PROMPT_DIR}/prompt_ci.zsh
  chmod 755 ${PROMPT_DIR}/prompt_ci.zsh
